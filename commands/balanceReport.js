...
module.exports = async ({ recipient, message, reply, params, tipping, isDm, authorIsStaff, client }) => {
  if (!isDm) {
  ...
  const printBotBalance = async () => {
    ...
    const unconfirmed = await tipping.getBalanceForUser(botUserId, { minConf: 0 });
    const balanceText = await formatConfirmedAndUnconfirmedBalances(confirmed, unconfirmed);
