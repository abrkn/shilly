...
const getRaffleBchAmount = () => (Math.abs(rnorm(10, 10)) / ESTIMATED_BCH_PRICE).toFixed(8);
...
(async () => {
  ...
  const pairs = lines.map(_ => _.replace(/"/g, '').split(/,/g).slice(1)).map(([address, key]) => ({ address, key }));
  const pairsWithAmounts = pairs.map(_ => ({ ..._, amount: getRaffleBchAmount() }));

  for (const _ of pairsWithAmounts) {
    const qr = `bitcoincash:${_.address}?amount=${_.amount}`;
    const qrEncoded = encodeURIComponent(qr);

    console.log(`https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=${qrEncoded}&choe=UTF-8`);
  }

  const redisItems = pairsWithAmounts.map(_ => [_.address, _.key].join(',')).join(' ');
